...
router.get('/search', auth, async (req, res) => {
  ...
  try {
    const result = await db.query(
      `SELECT id, full_name, university_email, university, avatar_url
       FROM users
       WHERE (full_name ILIKE $1 OR university_email ILIKE $1)
       AND id != $2
       LIMIT 10`,
      [`%${q}%`, req.user.id]
    );
    ...
    console.error('User search hatası:', err);
...
router.get('/me', auth, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id, full_name, university_email, university, department, avatar_url, bio, role, created_at FROM users WHERE id = $1',
      [req.user.id]
    );
